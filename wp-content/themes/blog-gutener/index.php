...
?>
	...
	if( is_home() && !get_theme_mod( 'disable_main_slider', false ) ){
		if ( get_theme_mod( 'main_slider_controls', 'slider' ) == 'slider' ){
			if ( get_theme_mod( 'display_slider_on', 'blog-page-above-latest-posts' ) == 'blog-page-below-header' || get_theme_mod( 'display_slider_on', 'blog-page-above-latest-posts' ) == 'front-blog-page-below-header' ) { ?>
	...
	<section class="section-post-area">
		<div class="row">
			<?php
				...
				if ( get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'right-left' ){
					$sidebarClass = 'col-lg-6';
					$sidebarColumnClass = 'col-lg-3';
				}
				if ( get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'no-sidebar' || get_theme_mod( 'disable_sidebar_blog_page', false ) ){
					$sidebarClass = 'col-12';
				}
				if( !get_theme_mod( 'disable_sidebar_blog_page', false ) ){
					if ( get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'left' || get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'right-left' ){ 
					?>
			...
			<div id="primary" class="content-area <?php echo esc_attr( $sidebarClass ); ?>">
				<?php if( is_home() && !get_theme_mod( 'disable_main_slider', false ) ){
					if ( get_theme_mod( 'main_slider_controls', 'slider' ) == 'slider' ){
						if ( get_theme_mod( 'display_slider_on', 'blog-page-above-latest-posts' ) == 'blog-page-above-latest-posts' ) { ?>
							<section class="section-banner">
								<?php 
									get_template_part( 'template-parts/slider/slider', '' ); 
								?>
				...
				<?php
					if( !get_theme_mod( 'disable_pagination', false ) ):
						the_posts_pagination( array(
							'next_text' => '<span>'.esc_html__( 'Next', 'blog-gutener' ) .'</span><span class="screen-reader-text">' . esc_html__( 'Next page', 'blog-gutener' ) . '</span>',
							'prev_text' => '<span>'.esc_html__( 'Prev', 'blog-gutener' ) .'</span><span class="screen-reader-text">' . esc_html__( 'Previous page', 'blog-gutener' ) . '</span>',
							'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'blog-gutener' ) . ' </span>',
						));
					endif;
				?>
			</div><!-- #primary -->
			<?php
				if( !get_theme_mod( 'disable_sidebar_blog_page', false ) ){
					if ( get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'right' || get_theme_mod( 'sidebar_settings', 'no-sidebar' ) == 'right-left' ){ ?>
